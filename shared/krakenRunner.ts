...
export function runKraken() {
    ...
    while (currentFileIndex < files.length) {
        ...
        const compileRes = spawnSync('npm', ['run', 'kraken-compile'], { stdio: 'inherit' });
        ...
        // Run Kraken test
        const testRes = spawnSync('npx', ['kraken-node', 'run'], { stdio: 'inherit' });
        ...
        if (testRes.status === 0) {
            ...
            attempts++;
            console.log(`Kraken test ${file} failed, retrying (Attempt ${attempts}/3)`);
            if (attempts > 3) {
